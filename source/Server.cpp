...
int Server::runServer()
{
	for (std::map<int, struct sockaddr_in>::iterator it = _sockets.begin(); it != _sockets.end(); it++)
	{
		...
		if (epollFd == -1)
		{
			Log::log(Log::ERROR, " Epoll create fail");
			return -1;
		}
		Log::log(Log::DEBUG, "success creating the epoll instance");	
		this->addSocket(epollFd,it->first, EPOLLIN);
		while (true)
		{
			Log::log(Log::INFO, "Waiting for new events.... (epoll loop)");	
			int num_fds = epoll_wait(epollFd, events, MAX_CO, -1); 
			Log::logVar(Log::DEBUG, "Number of events received:", num_fds);
			if (num_fds == -1)
			{
				Log::log(Log::ERROR, "Epoll error");
				return -1;
			}
...
void Server::handleEvent(epoll_event &event, int fd, int epollfd)
{
	try
	{
		...
		{
			Log::logVar(Log::INFO, "We received a connection ! the fd of the event is: ", event.data.fd);
			if (event.data.fd == fd)
			{
				Log::log(Log::DEBUG, "The event fd correspond to the server fd so we handle the connection");
				this->handleConnection(event.data.fd, epollfd);
			}
			else
			{
				Log::log(Log::DEBUG, "The event fd does not correspond to the server fd so we handle the request");
